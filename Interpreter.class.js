 ...
 *
 * Modified in 2025 - Converted from prototype-based to class-based implementation.
 * Original source: https://github.com/NeilFraser/JS-Interpreter
 */

/**
 * @fileoverview Interpreter for JavaScript, written in JavaScript.
 * @author Neil Fraser
 * @modified_by Sanithu Jayakody (sanithujayafiverr@gmail.com)
 */
'use strict';
...
class Interpreter {
   ...
   * E.g. NaN, Infinity.
   */
  static NONCONFIGURABLE_READONLY_NONENUMERABLE_DESCRIPTOR = {
    'configurable': false,
    'enumerable': false,
    'writable': false,
  };
   ...
   */
  static STEP_ERROR = { 'STEP_ERROR': true };

  /**
   * Unique symbol for indicating that a reference is a variable on the scope,
   * not an object property.
   */
  static SCOPE_REFERENCE = { 'SCOPE_REFERENCE': true };
   ...
   */
  static VALUE_IN_DESCRIPTOR = /** @type {!Interpreter.Value} */({
   ...
   */
  static REGEXP_TIMEOUT = { 'REGEXP_TIMEOUT': true };
   ...
   * @type {Interpreter}
   */
  static currentInterpreter_ = null;
   ...
   */
  static nativeGlobal = (typeof globalThis === 'undefined') ? this || window : globalThis;
  ...
  // Static methods for placeholder functions
  static placeholderGet_() {
    throw Error('Placeholder getter');
  }

  static placeholderSet_() {
  ...
  static Task = class {
    ...
    /** @type {number} Static counter for process IDs */
    static pid = 0;
  };
  ...
  constructor(code, opt_initFunc) {
     ...
     * @private
     */
    this.appendCodeNumber_ = 0;

    /**
     * Number of parsed tasks.
     * @private
     */
    this.taskCodeNumber_ = 0;

    /**
     * Number of functions created by the interpreter.
     * @private
     */
    this.functionCodeNumber_ = 0;

    /**
     * Number of eval code chunks.
     * @private
     */
    this.evalCodeNumber_ = 0;
   ...
   * @private
   */
  static stripLocations_(node, start, end) {
    if (start) {
      node.start = start;
    } else {
      delete node.start;
   ...
   */
  populateError(pseudoError, opt_message) {
    if (opt_message) {
   ...
   * @private
   */
  createFunctionBase_(argumentLength, isConstructor) {
   ...
   */
  nativeToPseudo(nativeObj, opt_cycles) {
   ...
   */
  pseudoToNative(pseudoObj, opt_cycles) {
  ...
  // stepIfStatement is an alias for stepConditionalExpression
  ...
  // stepNewExpression is an alias for stepCallExpression
  ...
  // stepWhileStatement is an alias for stepDoWhileStatement
  stepWhileStatement(stack, state, node) {
